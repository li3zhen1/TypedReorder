...
function sumrows(v: number[][]) {
    const n = v.length;
    const o = v[0].length;
    const sumrow = Array<number>(n);
...
function trans(y: number[]
    , yy: number[], x: number[], aidot: number[], mi: number, n: number, dat: number[][], prt?: any) {
    if (prt) {
...
function printvec(y: number[], prec: number, colperm: number[], line: string) {
    console.log('');
...
// @ts-ignore
function yxmult(y, x, mi, n, dat, prt?: any) {
    if (prt) {
        console.log('yxmult');
        printvec(x, 5, null, 'x=');
    }
