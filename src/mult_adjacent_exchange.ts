...
function count_all_in_crossings<N extends NodeLike, L extends LinkLike<N>>(
  graphs: Graph<N, L>[],
  a: N | number,
  b: N | number,
  layer: number[]
...
function count_all_out_crossings <N extends NodeLike, L extends LinkLike<N>>(
  graphs: Graph<N, L>[],
  a: N | number,
  b: N | number,
  layer: number[]
 ...
 */
export function mult_adjacent_exchange<N extends NodeLike, L extends LinkLike<N>>(
  graphs: Graph<N, L>[], layer1: number[], layer2: number[]) {
  layer1 = layer1.slice();
  layer2 = layer2.slice();
