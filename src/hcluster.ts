...
export function hcluster() {
    ...
    let linkage = 'single'; 
    let distMatrix: number[][] | null = null;

    function hcluster(vectors: number[][]) {
    ...
    hcluster.linkage = function (x: string) {
        if (!arguments.length) {
            return linkage;
        }
        linkage = x;
    ...
    hcluster.distance = function (x: any) {
        if (!arguments.length) {
            return distance;
        }
        distance = x;
    ...
    hcluster.distanceMatrix = function (x: number[][]) {
        if (!arguments.length) {
            // @ts-ignore
            return distMatrix;
        }
...
function calculateCentroid(c1Size: number, c1Centroid: number[], c2Size: number, c2Centroid: number[]) {
