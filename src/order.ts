...
export function order() {
    ...
    function _order_limits(i0: number, j0: number) {
        const orig = vector;
        ...
        if (i0 !== 0) {
            perm = permutation(i0).concat(perm.map((v: any) => v + i0));
        }
        if (orig.length > j0) {
    ...
    function _order() {
        ...
        const perm = (ordering().linkage(linkage) as any).distanceMatrix(distanceMatrix)(
            vector
        );
        if (_debug) {
    ...
    function _compute_dist() {
        if (distanceMatrix === null) {
            distanceMatrix = (dist().distance(distance) as any)(vector);
        }
