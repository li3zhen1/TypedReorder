...
export function count_in_crossings<N extends NodeLike, L extends LinkLike<N>>(
    graph: Graph<N, L>, 
    v: N | number,
    w: N | number,
    inv: number[]
...
export function count_out_crossings<N extends NodeLike, L extends LinkLike<N>>(
    graph: Graph<N, L>, 
    v: N | number,
    w: N | number,
    inv: number[]
 ...
 */
export function adjacent_exchange<N extends NodeLike, L extends LinkLike<N>>(graph: Graph<N, L>, layer1: number[], layer2: number[]) {
    layer1 = layer1.slice();
    layer2 = layer2.slice();
