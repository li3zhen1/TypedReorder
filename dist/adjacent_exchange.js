...
exports.adjacent_exchange = exports.count_out_crossings = exports.count_in_crossings = void 0;
...
function count_in_crossings(graph, v, w, inv) {
    ...
    var w_edges = graph.inEdges(w);
    var cross = 0;
    for (var iw = 0; iw < w_edges.length; iw++) {
        var t = w_edges[iw].target;
        var p0 = inv[w_edges[iw].target.index];
        for (var iv = 0; iv < v_edges.length; iv++) {
...
function count_out_crossings(graph, v, w, inv) {
    ...
    var w_edges = graph.outEdges(w);
    var cross = 0;
    for (var iw = 0; iw < w_edges.length; iw++) {
        var p0 = inv[w_edges[iw].source.index];
        for (var iv = 0; iv < v_edges.length; iv++) {
...
function adjacent_exchange(graph, layer1, layer2) {
    ...
    while (swapped) {
        ...
        for (var i = 0; i < layer1.length - 1; i++) {
            ...
            var c0 = count_out_crossings(graph, v, w, inv_layer2);
            var c1 = count_out_crossings(graph, w, v, inv_layer2);
            if (c0 > c1) {
                layer1[i] = w;
                layer1[i + 1] = v;
                inv_layer1[w] = i;
        ...
        for (var i = 0; i < layer2.length - 1; i++) {
            var v = layer2[i];
            var w = layer2[i + 1];
            var c0 = count_in_crossings(graph, v, w, inv_layer1);
            var c1 = count_in_crossings(graph, w, v, inv_layer1);
            if (c0 > c1) {
                layer2[i] = w;
                layer2[i + 1] = v;
                inv_layer2[w] = i;
